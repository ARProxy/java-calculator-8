...
- [ ] 요청 인자가 없을 경우 0을 반환한다.
- [ ] 쉼표 또는 클론 구분자로 문자열을 분리하고 합을 반환한다.
- [ ] 커스텀 구분자로 문자열을 분리하고 합을 반환한다
- [ ] 구분자로 입력된 문자열이 아닐 경우 잘못된 값으로 예외처리를 던진다.
- [ ] 숫자가 아닌 문자가 포함되면 `IllegalArgumentException`을 발생시킨다.
